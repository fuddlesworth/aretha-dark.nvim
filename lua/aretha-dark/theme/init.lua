...
local scheme_module = require("aretha-dark.theme.scheme")
local colors = require("aretha-dark.colors")

---@class ArethaDark.ThemeModule
local M = {}
...
local function build_highlights(scheme, config)
  ...
  local groups = require("aretha-dark.theme.groups")
  ...
  local plugins = require("aretha-dark.theme.plugins")
...
function M.build()
  ...
  local filter = config.filter or "default"
...
function M.load()
  ...
  if config.devicons then
    local ok, devicons = pcall(require, "aretha-dark.integrations.devicons")
    if ok then
      devicons.setup()
    end
  end
