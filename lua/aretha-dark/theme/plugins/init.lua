...
---@class ArethaDark.Theme.Plugins
local M = {}

local state = require("aretha-dark.theme.triggers")
local event_trigger = require("aretha-dark.theme.triggers.event")
local module_trigger = require("aretha-dark.theme.triggers.module")

--- Cache for loaded specs
---@type ArethaDark.PluginSpec[]|nil
local specs_cache = nil

--- Discover and load all plugin specs from this directory
---@return ArethaDark.PluginSpec[]
function M.load_specs()
  ...
  for _, file in ipairs(vim.fn.glob(plugin_dir .. "/*.lua", false, true)) do
    ...
    if name ~= "init" then
      local module_path = "aretha-dark.theme.plugins." .. name
...
---@param spec ArethaDark.PluginSpec
...
---@param scheme ArethaDark.Scheme
---@param config ArethaDark.Config
